...
class PriceBandTests(unittest.TestCase):
    ...
    def test_render_includes_band_and_history(self):
        ...
        self.assertIn('id="widerPrice"', map_page)
        self.assertIn("listing.price < 500000 || listing.price > 750000", map_page)
    ...
    def test_render_puts_only_first_128_rows_in_overview(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("async function loadAllRows(reason)", page)
            self.assertIn("if (await loadAllRows('searching')) applyFilters()", page)
            self.assertIn("if (await loadAllRows('sorting')) sortRowsBy(th, i)", page)
            self.assertIn('data-search="example 129"', batch)
